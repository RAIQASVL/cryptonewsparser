...
export class CryptoNewsParser extends BaseParser {
  constructor(options: ParserOptions) {
    super("cryptonews", cryptoNewsConfig, options);
    this.logMessage("CryptoNewsParser initialized");
  }
  ...
  protected async extractNewsItems(): Promise<NewsItem[]> {
    ...
    try {
      ...
      if (this.page) {
        ...
        this.logMessage(`Page HTML saved: ${htmlPath}`);
      }
  ...
  protected async extractArticleContent(url: string): Promise<string> {
    ...
    await this.page
      ...
      .catch(() => this.logMessage("Article content not found", "warn"));
  ...
  protected async extractNewsItemsAlternative(): Promise<NewsItem[]> {
    ...
    try {
      ...
      this.logMessage("RSS unavailable, trying direct extraction", "info");
  ...
  private async extractNewsFromMobile(): Promise<NewsItem[]> {
    ...
    try {
      ...
      this.logMessage(`Error extracting from mobile: ${error}`, "error");
      return [];
    }
  }
  ...
  protected async saveDebugScreenshot(name: string): Promise<void> {
    ...
    this.logMessage(`Debug screenshot saved: ${screenshotPath}`);
  }
}
