...
export class TheBlockParser extends BaseParser {
  constructor(options: ParserOptions) {
    super("theblock", theBlockConfig, options);
    this.logMessage("TheBlockParser initialized");
  }
  ...
  protected async extractNewsItems(): Promise<NewsItem[]> {
    ...
    try {
      ...
      this.logMessage(`Extracted ${articles.length} articles`);
      ...
      this.logMessage(`Found ${validArticles.length} valid articles`);
      ...
      try {
        ...
        this.logMessage(
          `Error in emergency extraction: ${emergencyError}`,
          "error"
        );
      }
  ...
  private async extractEmergencyData(): Promise<NewsItem[]> {
    ...
    try {
      ...
      this.logMessage(`Error in emergency data extraction: ${error}`, "error");
      return [];
    }
  }
  ...
  private async extractNewsItemsAlternative(): Promise<NewsItem[]> {
    ...
    try {
      ...
      if (isRssAvailable) {
        this.logMessage("RSS feed available, extracting news from it", "info");
        ...
        if (rssItems && rssItems.length > 0) {
          ...
          for (const item of rssItems.slice(0, 10)) {
            try {
              ...
              this.logMessage(`Error processing RSS item: ${error}`, "error");
            }
          }
      ...
      this.logMessage("RSS unavailable, trying search via Google", "info");
      ...
      if (searchResults && searchResults.length > 0) {
        ...
        for (const item of searchResults.slice(0, 10)) {
          try {
            ...
            this.logMessage(
              `Error processing search result: ${error}`,
              "error"
            );
          }
        }
      ...
      this.logMessage(
        "Failed to extract news using alternative methods",
        "error"
      );
      return [];
    } catch (error) {
      this.logMessage(`Error in alternative extraction: ${error}`, "error");
      return [];
    }
  }
  ...
  protected async extractArticleContent(url: string): Promise<string> {
    try {
      this.logMessage(`Navigating to article: ${url}`);
